...
const TechStack = () => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-[100px] left-1/2 -translate-x-1/2 w-[400px] h-[200px] bg-green-400 blur-[120px] opacity-10 rounded-full pointer-events-none" />

      {/* === Heading Wrapper with Background Text === */}
      <div className="relative z-10 mb-20 text-center">
        {/* Background “SKILLS” — top-left aligned */}
        <motion.h1
          style={{ y: yParallax }}
      ...
      {/* === Tech Cards Grid === */}
      <div className="relative z-10 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6 max-w-6xl mx-auto w-full">
        {techs.map(({ name, icon: Icon, color }, index) => (
          <motion.div
            key={name}
            className="relative group bg-white/5 border border-zinc-800 backdrop-blur-md p-6 rounded-3xl flex flex-col items-center justify-center text-center shadow-md cursor-pointer transition-all duration-300 ease-in-out hover:border-white/10"
            whileHover={{
              scale: 1.05,
              rotateX: 4,
              rotateY: 4,
          ...
          >
            {/* Glow Ring */}
            <div
              className="absolute -inset-1.5 z-0 rounded-[inherit] blur-xl opacity-0 transition duration-500 group-hover:opacity-100"
              style={{
            ...
            <div className="relative z-10 flex flex-col items-center">
              <Icon size={44} style={{ color }} className="transition-transform duration-300 group-hover:scale-110" />
              <p className="mt-4 text-zinc-300 text-xs md:text-sm font-semibold tracking-wide transition-colors group-hover:text-white">{name}</p>
            </div>
          </motion.div>
        ))}
      </div>
